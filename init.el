...
(global-set-key (kbd "s-k") 'candera/select-frame-by-name)
...
(defconst candera-ghostel-jump-to-last-input-max-pages 300
...
live tail) gets the same dim, low-contrast face -- (:foreground
\"#505050\" :background \"#373737\") -- with no distinguishable
\"arrived\" version; two earlier attempts at filtering on that face,
or waiting longer for it to change, were both confirmed wrong by
direct testing. What DOES distinguish a match worth landing on --
see `candera-ghostel--find-arrived-input-p' -- is whether Claude Code
is still showing it as a sticky reminder pinned to the screen's top
line, which it does for as long as you're scrolled anywhere within
that message's own (possibly very long, e.g. autonomous-mode) response,
regardless of the dim styling."
  (save-excursion
    (goto-char (point-max))
    (when (re-search-backward "^❯ " nil t)
      (match-beginning 0))))

(defun candera-ghostel--top-line-p (pos)
...
screen's very first line as a sticky reminder while you're scrolled
anywhere within that message's own response -- which, in an
autonomous/auto-mode session, can span hundreds of screens of
unattended agent work. The reminder is identical, unchanging, dim
text no matter how many of those screens you page through; it only
moves off line 1 (with genuinely earlier content now above it) once
you've scrolled back far enough to reach where that message actually
occurred. A match still pinned to line 1 is therefore not a real
arrival -- keep paging instead of stopping on it. (The one exception,
...
`candera-ghostel-jump-to-last-input-max-pages') until Claude Code
stops showing it as a sticky top-line reminder and it's genuinely
on-screen -- see `candera-ghostel--find-arrived-input-p'. Freezes the
view there via the read-only mode picked by
`ghostel-prompt-navigation-input-mode' (same as
